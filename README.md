...
3. **`iac-apply.yml`**: `apply-test`, then `apply-dev`, then `apply-prod` (`needs: apply-dev`,
   `environment: prod` -> **manual approval**). Only on `main`.
...
> - Set up Environments `test`, `dev` and `prod` (Settings -> Environments); add **required reviewers**
>   to `prod` for the manual gate.
...
- **Naming via the `label` module** - every resource name is generated from `project` + `stage` +
  ...
  convention, since the Terragrunt `remote_state` block cannot call a Terraform module.
...
- **Identity -> AKS**: the cluster consumes the `identity` module's managed identity and is granted
